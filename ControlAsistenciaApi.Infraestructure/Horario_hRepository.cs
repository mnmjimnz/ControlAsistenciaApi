...
namespace ControlAsistenciaApi.Infraestructure
{
    public class Horario_hRepository: IHorario_hRepository
    {
        ...
        public async Task<int> GuardarHorario_h(Horario_h p)
        {
            try
            {
                string sql = "INSERT INTO horario_h(idaula, idmateria, hora_inicio, hora_fin, fecha, catedratico) VALUES(@idaula, @idmateria, @hora_inicio, @hora_fin, @fecha, @catedratico) SELECT SCOPE_IDENTITY();";
        ...
        public async Task<int> EditarHorario_h(Horario_h p)
        {
            try
            {
                string sql = "UPDATE horario_h SET idaula = @idaula, idmateria = @idmateria, hora_inicio = @hora_inicio, hora_fin = @hora_fin, fecha = @fecha, catedratico = @catedratico WHERE id = @id";
