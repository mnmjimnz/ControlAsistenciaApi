...
namespace ControlAsistenciaApi.Controllers
{
    ...
    public class HorarioHController : ControllerBase
    {
        ...
        [HttpGet("ObtenerHorarioPorId")]
        public async Task<IActionResult> ObtenerHorarioPorId(int id)
        {
            try
            {
                return Ok(await _horario_hUseCase.ObtenerHorario_hPorId(id));
            }
            catch (Exception ex)
            {
                return BadRequest(ex);
        ...
        [HttpGet("ObtenerHorarioPorIdAula")]
        public async Task<IActionResult> ObtenerHorario_hPorIdAula(int idAula, int PageSize, int PageNumber)
        {
            try
            {
                return Ok(await _horario_hUseCase.ObtenerHorario_hPorIdAula(idAula, PageSize, PageNumber));
            }
            catch (Exception ex)
            {
                return BadRequest(ex);
        ...
        [HttpGet("ObtenerHorarioPorDiaYAnio")]
        public async Task<IActionResult> ObtenerHorario_hPorDiaYAnio(string dia, string anio, int PageSize, int PageNumber)
        {
            try
            {
                return Ok(await _horario_hUseCase.ObtenerHorario_hPorDiaYAnio(dia, anio, PageSize, PageNumber));
            }
            catch (Exception ex)
            {
                return BadRequest(ex);
