...
namespace ControlAsistenciaApi.Usecase
{
    public class Horario_hUseCase: IHorario_hUseCase
    {
        ...
        public async Task<List<Horario_hDto>> ObtenerHorario_hPorDiaYAnio(string dia, string anio, int PageSize, int PageNumber)
        {
            try
            {
                var r = await _repoHorario_h.ObtenerHorario_hPorDiaYAnio(dia, anio ,PageSize, PageNumber);
                return _mapper.Map<List<Horario_hDto>>(r);
            }
            catch (Exception ex)
            {
        ...
        public async Task<List<Horario_hDto>> ObtenerHorario_hPorDiaAnioAulaCicloMateria(string dia, string anio, int? aula, int? materia, string ciclo, int PageSize, int PageNumber)
        {
            try
            {
                var r = await _repoHorario_h.ObtenerHorario_hPorDiaAnioAulaCicloMateria(dia, anio, aula, materia, ciclo, PageSize, PageNumber);
                return _mapper.Map<List<Horario_hDto>>(r);
            }
            catch (Exception ex)
            {
